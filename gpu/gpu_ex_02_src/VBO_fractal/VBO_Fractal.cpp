...
// TODO: Generiere eine Display-Liste
void drawDisplayList()
{
	// TODO: Benutze eine fertige Display-List zum Zeichnen
...
void display()
{
	...
	if (drawMode == VBO) {    // fast drawing
		drawGeometryVertexBuffer();
	}
	else if(drawMode == IMMEDIATE) {  // simple and slow drawing
		glBegin(GL_TRIANGLES);
		for (unsigned int i = 0 ; i < positions.size() ; i += 3) {
			glNormal3fv(&normals[i][0]);
			glVertex3fv(&positions[i][0]);
		...
		glEnd();
	} else {	// Simple drawing with displaylist
		drawDisplayList();
	}
